...
| CR-5.6 | [x] | [x] | [x] | As a developer, I can have commits whose messages begin with "Set dev version" automatically excluded from the release notes draft so that internal version-restoration commits from `merge-to-dev` do not appear as release note bullets. Acceptance: the housekeeping-commit filter in CR-5.1 includes a case-insensitive pattern matching `/^set dev version/i`; any commit matching this pattern is silently dropped from the draft regardless of the remaining message text. |
